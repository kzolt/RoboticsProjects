...
#define ONE_FLOOR_TIME 1000 	// Time to get to move floor (milliseconds)
...
void move_elevator()
{
	...
	motor[RightMotor] = (elv.dir == DIR_UP ? elv.speed + 2 : -elv.speed - 2);
}
...
task go_to_floor()
{
	...
	while (!isEmpty(elv.q))
	{
		...
		// Individual instructions for "One" floor movemnt
		if (elv.current_floor == FLOOR_ONE   && ftg == FLOOR_TWO 	||
			elv.current_floor == FLOOR_TWO   && ftg == FLOOR_ONE 	||
			elv.current_floor == FLOOR_TWO   && ftg == FLOOR_THREE	||
			elv.current_floor == FLOOR_THREE && ftg == FLOOR_TWO)
				delay(ONE_FLOOR_TIME);

		// Individual instructions for "Two" floor movement
		if (elv.current_floor == FLOOR_THREE && ftg == FLOOR_ONE ||
		    elv.current_floor == FLOOR_ONE   && ftg == FLOOR_THREE)
				delay(ONE_FLOOR_TIME * 2);

		// Stops the elevator and sets current floor to be the floor we just traveled to, also removes the floor from our queue
		elv.dir = DIR_NONE;
		move_elevator();
		elv.current_floor = front(elv.q);
		dequeue(elv.q);
